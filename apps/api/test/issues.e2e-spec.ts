...
describe('Issues e2e：问题清单权限矩阵、状态机与指派', () => {
  ...
  beforeAll(async () => {
    ...
    // PM 先按 key user 邀请，再升级为 customer_pm（JWT 登录时签发，须升级后重新登录）
    ...
    pmToken = await login('pm@tenant-a.test');
    keyUserToken = await inviteMember(projectAId, { email: 'ku@tenant-a.test', role: 'customer_key_user' });
