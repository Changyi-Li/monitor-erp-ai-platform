...
export async function resetTestDb(): Promise<void> {
  ...
  await client`TRUNCATE users, refresh_tokens, customers, projects, user_tenants, project_members, issues, issue_comments, issue_links, blueprints, blueprint_versions, project_stages, project_risks, meeting_minutes, minute_attachments, kb_documents, kb_document_versions, manual_generations, manual_chapters, import_staged_documents, document_syncs, ai_conversations, ai_messages, langgraph_checkpoints, langgraph_checkpoint_writes, ai_usage, audit_logs`;
  await client.end();
}
